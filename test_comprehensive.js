...
const testNameParsing = () => {
  ...
  function parseIndividualName(name) {
    ...
    if (name.includes(",")) {
      ...
      console.log(
        `    Comma split: lastPart="${lastPart}", firstPart="${firstPart}"`
      );
      ...
      if (firstPart) {
        ...
        console.log(
          `    First parts: [${firstParts.map((p) => `"${p}"`).join(", ")}]`
        );
        ...
        if (
          ...
          console.log(
            `    First part "${firstParts[0]}" is an initial, looking for substantial part...`
          );
